package org.sean.mlbook.widget.contentswitchview;
...
import org.sean.mlbook.ReadBookControl;
import org.sean.mlbook.widget.MTextView;
...
public class BookContentView extends FrameLayout {
    ...
    private void init() {
        ...
        tvContent = (org.sean.mlbook.widget.MTextView) view.findViewById(R.id.tv_content);
    ...
    public void updateData(long tag, String title, List<String> contentLines, int durChapterIndex, int chapterAll, int durPageIndex, int durPageAll) {
        if (tag == qTag) {
            ...
            finishLoading();
            if (durPageIndex % 40 == 10) {
                // TODO 加广告
                Toast.makeText(getContext(), "此功能需要解锁,请观看广告支持一下", Toast.LENGTH_SHORT).show();
                GAD.showCover(BaseActivity.instance);
            }
        }
    }
